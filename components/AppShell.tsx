...
import { ArrowUpTrayIcon, BellAlertIcon, BriefcaseIcon, CalendarDaysIcon, ChatBubbleLeftRightIcon, Cog6ToothIcon, HomeIcon, MoonIcon, PlusIcon, SunIcon, UserCircleIcon, UsersIcon } from "@heroicons/react/24/outline";
  ...
  return <div className="min-h-screen lg:grid lg:grid-cols-[264px_1fr]">
    ...
    <div className="min-w-0">
      <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-black/[.06] bg-sand/85 px-4 backdrop-blur-md lg:px-8">
        ...
        <div className="flex gap-2"><button type="button" onClick={toggleTheme} aria-label={`Use ${dark ? "light" : "dark"} appearance`} className="grid size-10 place-items-center rounded-xl border border-black/10 bg-white text-ink"><>{dark ? <SunIcon className="size-5" /> : <MoonIcon className="size-5" />}</></button><span className={`hidden items-center rounded-full px-3 py-2 text-xs font-black sm:inline-flex ${online ? "bg-forest text-white" : "bg-orange-100 text-orange-800"}`}>{online ? "Online" : "Offline"}</span><RoleBadge /><LogoutButton /></div>
      </header>
...
function NavLink({ href, active, children }: { href: string; active: boolean; children: React.ReactNode }) {
  return <Link href={href} className={`flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm font-semibold transition ${active ? "bg-white/12 text-lime shadow-sm" : "text-white/65 hover:bg-white/5 hover:text-white"}`}>{children}</Link>;
}
