...
function normalizeSettings(input: Partial<BusinessSettings>): BusinessSettings {
  return {
    ...
    appDisplayName: input.appDisplayName?.trim() || "Company Command",
    headerName: input.headerName?.trim() || "Company Command — RTS",
    brandShortName: input.brandShortName?.trim() || "CC",
    companyName: input.companyName?.trim() || "RTS Field App",
    phone: input.phone?.trim() || "",
    email: input.email?.trim() || "Texastrimout@gmail.com",
    address: input.address?.trim() || "",
    city: input.city?.trim() || "",
    defaultCalendar: input.defaultCalendar?.trim() || "Google Calendar",
    defaultState: input.defaultState?.trim() || "TX",
    ...
    customerTextTemplate: input.customerTextTemplate?.trim() || "RTS update for {customerName}: crew is on your job {jobId}.",
